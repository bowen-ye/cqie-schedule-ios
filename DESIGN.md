...
School credentials are entered only on official CQIE pages. Native clients read the timetable directly from official APIs. Because the school rejects external OAuth callbacks and its cross-origin headers fail browser checks, Safari onboarding uses a user-created bookmarklet to read the current-term schedule while it is still on the official origin. Only compressed schedule data crosses through a URL fragment; the fragment is cleared immediately and must carry the nonce generated by that device's bookmark setup. Since iOS isolates Home Screen Web App storage, the install action opens a dedicated no-manifest page and copies a sanitized schedule transfer containing no account identifier or login token. The saved fragment is an automatic fast path; a visible clipboard action is the fallback and update path. Both paths validate the snapshot transactionally before rendering it in the isolated Web App.
